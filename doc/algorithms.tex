...
the following three term recursion:
...
recurrence is not evaluated. These two cases are:
\begin{itemize}
\item[(a)] The base cases $P_0$ and $P_1$.
\item[(b)] If $n \ge 1$ is odd, then $P_n(0) = 1$.
\end{itemize}
...
$g_i$, $h_i$, $q_i$, $b_i$, $a_i$ the values of the variables $f$, $g$, $h$,
$q$, $b$, $a$ computed during loop $i$. We are interested in the final value
$a_n$, which approximates $P_{n+1}(x)$.
...
first initialization of the value $a$. Here, we use the
...
At the end of the loop, we will get an approximation $a_n$ of $P_{n+1}(x)$ such
that:
\[ |a_n - P_{n+1}(x)| \le 2^{e_{a,n}}. \]
This error bound will be used in the rounding test, and if it fails, we will try
again with a larger working precision $p' > p$.
