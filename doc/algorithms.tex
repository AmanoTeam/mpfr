...
polynomials defined in the domain $(-\infty,\infty)$.
...
\begin{equation} \label{eq:hermite-recursion}
  ...
  \end{cases}
\end{equation}
Some values of $x$ and $n$ are treated as special cases. These cases are:
\begin{itemize}
...
\end{itemize}
The fraction in (c) is computed with the Log-Gamma method to avoid overflow for
large $n$. Let's start from the implementation of the case $H_{2k}(0)$ and its
error analysis.
\vspace{.3cm}
...
\begin{algorithm}[H]
  ...
  ternary\_value $\gets$ mpfr\_set (res, e, rnd\_mode)\;
  \lIf{$~~((n>>1)\&1)~~$}{
    MPFR\_SET\_NEG (res)\;
  }
  \Return ternary\_value\;
  \BlankLine
\end{algorithm}
...
except for the last division, that is missing here. For that reason, we skip
the analysis step by step of the first iteration of the recurrence, and we just
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$p_2^1 \gets p_1^0$
& $k_{p_2^1} \le k_{p_1^0};\quad k_{p_2^1} \le \frac12$ \\[4pt]
