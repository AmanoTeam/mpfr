...
Hermite polynomials $H_n(x)$, with $n \geq 0$, are a sequence of orthogonal
polynomials defined in the domain $(-\infty,\infty)$. They have a special
interest in various branches of physics, e.g., in defining eigenstates
of the quantum harmonic oscillator.

In this document, $H_n(x)$ refers to the physicist's convention -- the
ones implemented by \texttt{mpfr\_hermite}; this differs from the
probabilist's convention, which differs by scaling factors.

MPFR does not use Rodrigues' formula to compute Hermite polynomials
...
\begin{equation} \label{eq:hermite-recursion}
  ...
  \end{cases}
\end{equation}
The polynomial $H_n(x)$ is even for $n$ even, and odd for $n$ odd,
i.e., $H_n(-x) = (-1)^n~H_n(x)$.
...
from the fact that $H_n$ is odd for $n$ odd:
\begin{itemize}
\item[(a)] The base cases $H_0$ and $H_1$.
\item[(b)] If $n$ is odd, then $H_n(0) = 0$.
\end{itemize}
...
$a_i$ approximates $H_i(x)$ and $b_i$ approximates $H_{i-1}(x)$, and
$g_i$, $h_i$, $q_i$, $b_{i+1}$, $a_{i+1}$ the values of the variables $g$, $h$,
$q$, $b$, $a$ computed during loop $i$. We are interested in the final value
$a_n$, which approximates $H_n(x)$.
...
\par\noindent
\textbf{Analysis of $a = \N{}(2\cdot{}x)$.} This is the
first initialization of the value $a$, say $a_1$, which approximates $H_1(x)=2x$.
Here, we use the
\texttt{mpfr\_mul\_2ui} function to multiply the exact input $x$ by 2.
If the precision of $a$ is less than that of $x$,
...
\par\noindent
\textbf{Analysis of $f~\gets~a$.} Since this operation is exact ($f$ and $a$ have
...
\par\noindent
\textbf{Analysis of $h_i = \N{}(f\cdot{}a_i)$.}
Here we have three sources of error: the rounding error on $h_i$,
the induced error on $f$, which approximates $2x$,
and the induced error on $a_i$, which approximates $H_i(x)$:
\begin{eqnarray*}
  |h_i - 2x H_i(x)| & \le &
  |h_i - f a_i| + |f a_i - 2x a_i| + |2x a_i - 2x H_i(x)| \\
  & \le & 2^{\Exp(h_i)-p-1} + 2^{e_{f} + \Exp(a_i)} + 2^{1 + \Exp(x) + e_{a,i}}.
\end{eqnarray*}
Since $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{h,i} \le 2 + \max(\Exp(h_i)-p-1, e_{f} + \Exp(a_i),
  1 + \Exp(x) + e_{a,i}). \]

\par\noindent
\textbf{Analysis of $q_i = \N{}(h_i - g_i)$.}
Here again, we have three sources of error: the rounding error on $q_i$,
the induced error on $h_i$, which approximates $H_i := 2x H_i(x)$,
and the induced error on $g_i$, which approximates $G_i := (2i)H_{i-1}(x)$:
\[ |q_i - (H_i - G_i)| \le 2^{\Exp(q_i)-p-1} + 2^{e_{h,i}} + 2^{e_{g,i}}. \]
Using again $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{q,i} \le 2 + \max(\Exp(q_i)-p-1, e_{h,i}, e_{g,i}). \]

At the end of the loop, we will get an approximation $a_n$ of $H_n(x)$ such
that:
\[ |a_n - H_n(x)| \le 2^{e_{a,n}}. \]
This error bound will be used in the rounding test, and if it fails, we will try
again with a larger working precision $p' > p$.

\paragraph{\textbf{Overflow handling.}}

This paragraph details how overflow is handled.
Each polynomial $H_n$ has $n$ distinct real roots, each of them lies in the
closed interval $[-\sqrt{2n+1}, \sqrt{2n+1}]$, that is called the
\emph{oscillatory region} of the polynomial $H_n(x)$.

The values of $H_n(x)$ grow very rapidly as $|x|$ increases beyond $\sqrt{2n+1}$,
...
all subsequent steps, i.e., if $H_n(x)$ overflows, the same happens to every
$H_i(x)$ with $i > n$.

\noindent Consider the recurrence $H_{n+1}(x) = 2x H_n(x) - 2n H_{n-1}(x)$.
...
least a factor $x$. In particular $H_i(x) > x^i > 0$, and the sequence
$|H_i(x)|$ is strictly increasing with $i$. This means that, outside
the oscillatory region, i.e., $x^2 > 2i + 1$, if $H_i(x)$ overflows,
so does $H_{i+1}(x)$.

\begin{proof}{(by induction on $i$)}
For the base case $i = 1$: $H_1(x)/H_0(x) = 2x > x$ since $x > 0$.
For the inductive step, assume $H_i(x)/H_{i-1}(x) > x$ for some
$i \ge 1$ with $x^2 > 2i$.
From the recurrence:
\[
  \frac{H_{i+1}(x)}{H_i(x)} = 2x - 2i \, \frac{H_{i-1}(x)}{H_i(x)}.
\]

By the induction hypothesis, $H_i(x)/H_{i-1}(x) > x > 0$, hence
$0 < H_{i-1}(x)/H_i(x) < 1/x$. Therefore:
\[
  \frac{H_{i+1}(x)}{H_i(x)} > 2x - \frac{2i}{x}.
\]
This is greater than $x$ if and only if $x > 2i/x$, i.e., $x^2 > 2i$,
which holds by hypothesis. This completes the induction.
\end{proof}

\begin{corollary}
If $x > 0$ with $x^2 > 2n$, and if $H_i(x)$ overflows for some
$1 \le i \le n$, then $H_m(x)$ overflows for all $m$ with
$i \le m \le n$.
\end{corollary}

In the implementation, computations are performed in extended exponent
range. An overflow in this range
...
intermediate overflow in the same way, considering the final value
$H_n(x)$ overflows, both when $x$ is in the oscillatory region or not.

\subsubsection{Handling small inputs}

As for Legendre polynomials, when $|x|$ is very small, $H_n(x)$ can be well
...
\ref{subsec:legendre-small-inputs}.
Let $c_k$ be the coefficient of degree $k$ of $H_n(x)$,
or $c_{k,n}$ in case of ambiguity.
We want to bound the ratios of the first terms
...
For odd degrees, we set $k = 1$, and we get $6c_3 = -2(n-1)c_1$. Hence,
\[
  \left|\frac{c_3}{c_1}\right| = \frac{n-1}{3} \le \frac{n}{3}.
\]

Now, for even degrees we set $k = 0$ and we get $2c_2 = -2n\,c_0$. Hence,
\[
...
\paragraph{\textbf{Implementation.}} The trailing term (i.e., that of smallest
degree) is evaluated differently
depending on the parity of $n$:
\begin{itemize}
\item If $n$ is even, the trailing term is $c_{0,n} = H_n(0)$, computed by
  \[
    c_{0,0}=1, \qquad c_{0,j}=-2(j-1)c_{0,j-2},
    \quad j = 2, 4, ..., n,
  \]
  thus
  \[
    c_{0,n} = (-1)^{n/2} \frac{2 (n-1)!}{(n/2-1)!}.
  \]

\item If $n$ is odd, the trailing term is $c_{1,n} x$, computed by
  \[
    c_{1,1}=2, \qquad c_{1,j}=-2jc_{1,j-2},
    \quad j = 3, 5, ..., n,
  \]
  thus
  \[
    c_{1,n} = (-1)^{(n-1)/2} \frac{2 n!}{((n-1)/2)!}.
  \]
\end{itemize}
We remark that for $n$ odd, the trailing coefficient of $H_n$
is the same (up to sign) as the one of $H_{n+1}$.
...
The condition $\rho \le -2$ guarantees
$t < 2^\rho \le 2^{-2} = \tfrac{1}{4}$.
The relative error from keeping only the trailing term equals $t/(1-t)$.
Since $t < \tfrac{1}{4} < \tfrac{1}{2}$, we have $1 - t > \tfrac{1}{2}$,
and therefore
\[
  \frac{t}{1-t} < \frac{t}{1/2} = 2t < 2 \cdot 2^\rho = 2^{\rho+1}.
\]
Truncating the expansion after the first nonzero term yields at least
$-\rho - 1$ correct bits relative to that term.

In the implementation, if $p$ is the precision of the result, we try this
shortcut only when
\[
  -\rho - 1 \ge p + 64.
\]
...
\paragraph{\textbf{Error analysis of $c_{0,n}$ and $c_{1,n}$}}
