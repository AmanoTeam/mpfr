...
These functions implement a \textit{non-cryptographic} hash function, based on the
Fowler-Noll-Vo function (from now on FNV), as detailed in Algorithm \ref{algo:fnv32}.
...
\begin{algorithm}[H]
  ...
  \For{$\ivar\gets 0$ \KwTo buffer\_size - 1}{
    byte $\gets$ buffer[i]\;
