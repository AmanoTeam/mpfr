...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Error analysis for the special case
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\par\noindent
\textbf{Analysis of $a_0 = \N{}(2\cdot{}x)$.} Here, we use the
\texttt{mpfr\_mul\_ui} function, with exact input $x$ and 2 (we assume 2
fits into a long). There is thus only one error, due to the rounding in
$\N{}(2\cdot{}x)$. This error is bounded by
$\frac{1}{2} \ulp(a_0) \le \frac{1}{2} 2^{\Exp(a_0) - p}$:
\[ e_{a_0} \le \Exp(a_0) - p - 1. \]

\par\noindent
\textbf{Analysis of $f_i = \N{}(2\cdot{}x)$.} As for $a_0$, we are using the
\texttt{mpfr\_mul\_ui} function, so there is only one error, due to the
final rounding. This error is bounded by
...
\begin{eqnarray*}
  |h_i - 2x P_{i-1}(x)| & \le &
  |h_i - f_i a_i| + |f_i a_i - 2x a_i| + |2x a_i - 2x P_{i-1}(x)| \\
  & \le & 2^{\Exp(h_i)-p-1} + 2^{e_{f,i} + \Exp(a_i)} + 2^{2 + \Exp(x) + e_{a,i}}.
\end{eqnarray*}
Since $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{h,i} \le 2 + \max(\Exp(h_i)-p-1, e_{f,i} + \Exp(a_i),
  2 + \Exp(x) + e_{a,i}). \]
...
\textbf{Analysis of $q_i = \N{}(h_i - g_i)$.}
Here again, we have three sources of error: the rounding error on $q_i$,
the induced error on $h_i$, which approximates $H_i := 2x P_{i-1}(x)$,
and the induced error on $g_i$, which approximates $G_i := (2i)P_{i-2}(x)$:
\[ |q_i - (H_i - G_i)| \le 2^{\Exp(q_i)-p-1} + 2^{e_{h,i}} + 2^{e_{g,i}}. \]
Using again $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{q,i} \le 2 + \max(\Exp(q_i)-p-1, e_{h,i}, e_{g,i}). \]
