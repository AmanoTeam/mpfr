...
\begin{algorithm}[H]
  ...
  \While{$i\leq{}n$}{
    ...
    \tcc{Here we perform a fused multiplication and subtraction, to minimize the rounding error}
    $p_i~\gets~\nabla{}(\mathrm{first\_term}\cdot{}p_1 - \mathrm{second\_term})$\;
    $p_i~\gets~\nabla{}(p_i/i)$\;
    \tcc{Here we use mpfr\_swap to avoid unnecessary copies}
    $p_2~\gets~p_1$\;
    $p_1~\gets~p_i$\;
    $i++$\;
  }
  \tcc{Final rounding of the result with the input rounding mode}
  ret $\gets$ mpfr\_set(res, p\_i, rnd\_mode)\;
  \Return ret;
  \BlankLine
\end{algorithm}
\vspace{.3cm}

\noindent In Algorithm \ref{algo:legendre-bonnet} above, you can find the pseudo-code
implementation of the \textit{iterative} Bonnet's recursion with final rounding.
