...
holds whenever $ku \le \tfrac{1}{2}$, i.e. $p \ge 1 + \lceil\log_2 k\rceil$,
which is satisfied by our choice of working precision.

\subsection{Hermite polynomials}

Hermite polynomials $P_n(x)$, with $n \geq 0$, are a sequence of
orthogonal polynomials defined in the domain $(-\infty,\infty)$. These
polynomials arise naturally in physics, especially in problems
involving Gaussian weights—most famously in the quantum harmonic
oscillator.

In this document, $P_n(x)$ refers to the physicist's convention; this
differs from the probabilist's convention, which is often denoted by
$H_n(x)$ and is related to $P_n(x)$ by a scaling factor. The
explicit physicist's Hermite polynomials are calculated by using the
Rodrigues formula. Here's the first (physicist's) Hermite polynomials
\begin{align*}
  P_0(x) &= e^{x^2} \frac{d^0}{dx^0} e^{-x^2} = 1 \\
  P_1(x) &= -e^{x^2} \frac{d}{dx} e^{-x^2} = -e^{x^2} \cdot -2x e^{-x^2} = 2x \\
  P_n(x) &= (-1)^n e^{x^2} \frac{d^n}{dx^n} e^{-x^2}
\end{align*}

MPFR does not use the Rodrigues formula to compute Hermite
polynomials, but instead relies on a three-term recursion, which is
more efficient and numerically stable for this purpose.
...
interval $[-\sqrt{2n+1}, \sqrt{2n+1}]$, that is called the
...
Inside the \emph{oscillatory region} the polynomial changes sign and
its values can decrease in magnitude from one step to the next, so the
monotonic growth argument does not hold within this interval.
