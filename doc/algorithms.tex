...
approximation and expansion methods. These polynomials are computed by MPFR
...
\begin{algorithm}[H]
  ...
  \BlankLine
  \tcc{The higher degree allowed is $2^{13}$}
  \lIf{$~~(n>8192)~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
  \tcc{We have to check the domain bounds of the parameter $x$}
  \lIf{$~~(x<-1~\mathbf{or}~x>1)~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
...
\begin{algorithm}[H]
  ...
  \KwResult{ternary value}
  \BlankLine
  \tcc{Initializing all the variables to the working precision (realprec)}
  init\_prec (realprec, $p_1$, $p_2$, $p_i$, first\_term, second\_term)\;
  \tcc{Initializing the two base cases}
  $p_2~\gets{}~1$\;
  $p_1~\gets~\N(x)$\;
  $i~\gets~2$\;
  \tcc{Loop from 2 to $n$, where $n$ is the degree of the polynomial}
  \While{$i\leq{}n$}{
    $\mathrm{first\_term}~\gets~\N{}(x\cdot{}(2i - 1))$\;
    $\mathrm{second\_term}~\gets~\N{}(p_2\cdot{}(i - 1))$\;
    $\mathrm{first\_term}~\gets~\N{}(\mathrm{first\_term}\cdot{}p_1)$\;
    \tcc{Check for cancellation in the subtraction}
    $p_i~\gets~\N{}(\mathrm{first\_term} - \mathrm{second\_term})$\;
    $p_i~\gets~\N{}(p_i/i)$\;
    $p_2~\gets~p_1$\;
    $p_1~\gets~p_i$\;
    $i++$\;
  }
  \tcc{Final rounding of the result with the input rounding mode}
  \Return mpfr\_set(res, $p_i$, rnd\_mode)\;
  \BlankLine
\end{algorithm}
\vspace{.3cm}

\noindent In Algorithm \ref{algo:legendre-bonnet} above, you can find the
pseudo-code implementation of the \textit{iterative} Bonnet's recursion
with final rounding. Now, we have to bound the rounding error for the
$n$-th iteration of the loop $\epsilon_n$. Let's start from the first
initialization of $p_1$ and $p_2$, that is
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$p_1^0 \gets \N(x)$
& $k_{p_1^0} \le \tfrac12$, since we are rounding to the nearest. \\[4pt]
$p_2^0 \gets 1$
& $k_{p_2^0}=0$, since $1$ is exact. \\[6pt]
\end{tabular}
\end{center}
Then, in the first iteration of the loop we have
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$\text{first\_term} \gets \N((2i-1)\,x)$ &
Multiplication by an exact integer: $c_{\text{first}}=\tfrac12$, so
$k_{\text{first}} = k_2 = \tfrac12$.\\[4pt]
$\text{first\_term} \gets \N(\text{first\_term}\cdot p_1^0)$
& Using the generic multiplication rule:
$k_3 \le c_3 + (1+c_{p_1^0}^+)k_{\text{first}} + (1+c_{\text{first}}^+)k_{p_1^0}$,
with $c_3=\tfrac12$ and $c_{p_1^0}^+=c_{\text{first}}^+=1$.
Thus $k_3 \le \tfrac12 + 2(k_{p_1^0}+k_2) = \tfrac12 + 2\Bigl(\tfrac12+\tfrac12\Bigr) = \tfrac52$. \\[4pt]
$\text{second\_term} \gets \N((i-1)p_2^0)$ &
Integer multiplication followed by rounding:
$k_4 \le c_4 + (1+c_{p_2^0}^+)k_{p_2^0}$,
with $c_4=\tfrac12$, $c_{p_2^0}^+=1$, $k_{p_2^0}\le \tfrac12$. Thence:
$k_4 \le \tfrac12 + 2k_{p_2^0} \le \tfrac32.$ \\[4pt]
$p_i \gets \N(\text{first\_term} - \text{second\_term})$ &
Using the generic subtraction rule (same sign operands):
$k_{p_i} \le c_{p_i} + k_3 + k_4, c_{p_i}=\tfrac12$. Thus:
$k_{p_i} \le \tfrac12 + \tfrac52 + \tfrac32 = \tfrac92$.
A runtime heuristic it's applied to handle eventual cancellation. \\[4pt]
$p_i \gets \N(p_i/i)$ &
Exact integer division. Using generic division:
$k_{p_i}' \le c_{p_i'} + 2k_{p_i}, c_{p_i'}=\tfrac12$. Thus,
$k_{p_i}' \le \frac12 + 2\cdot \frac92 = \frac{19}{2}$.
\\[4pt]
\end{tabular}
\end{center}
At this point we perform the swap (lines 12--13 in Algorithm \ref{algo:legendre-bonnet}),
therefore
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
 $p_2^1 \gets p_1^0$
& $k_{p_2^1} \le k_{p_1^0};\quad k_{p_2^1} \le \frac12$ \\[4pt]
$p_1^1 \gets p_i$
& $k_{p_1^1} \le k_{p_i};\quad k_{p_1^1} \le \frac{19}{2}$ \\[4pt]
\end{tabular}
\end{center}
\vspace{.2cm}

\paragraph{\textbf{Generic error bound for one iteration.}}

Let $k_{p_1^i}$ and $k_{p_2^i}$ denote the relative--error coefficients of
the two Legendre values at the beginning of iteration~$i$, we define
$a_i := k_{p_1^i}$ and $b_i := k_{p_2^i}$.

From the error analysis of the first iteration, replacing
the initial bounds by $a_i$ and $b_i$, we obtain the following recurrence
for the error coefficients after the division by $i$ and the subsequent
variable swap:
\[
  a_{i+1} \;\le\; 4a_i + 4b_i + \tfrac{11}{2}, \qquad
  b_{i+1} = a_i.
\]
Substituting $b_i$ yields the second-order recurrence of the form
\begin{equation} \label{eq:legendre-recurrence}
\left\{
\begin{aligned}
 &a_0 \le \frac12\\
 &a_1 \le \frac{19}{2}\\
 &a_{i+1} \le 4a_i + 4a_{i-1} + \frac{11}{2}
 \end{aligned}
 \right.
\end{equation}

We have the inhomogeneous inequality $a_{i+1}\le 4a_i+4a_{i-1}+\tfrac{11}{2}$,
so the associated homogeneous recurrence is $a_{i+1} = 4a_i + 4a_{i-1}$. Looking
for solutions of the form $a_i = r^i$, gives the characteristic polynomial $r^2 - 4r - 4 = 0$,
which has the roots $r_{1,2} = 2 \pm 2\sqrt2$.
Hence the homogeneous solution is
\[
 a_i^{(h)} = A(2+2\sqrt2)^i + B(2-2\sqrt2)^i.
\]
In order to obtain the full solution, we add to the homogeneous solution we got,
the constant particular solution given by $C = 4C + 4C + \frac{11}{2}$, where we
indicated with $C$ each $a_i$, and we get $a_i^{(p)} = -\tfrac{11}{14}$.
Thus the full solution is
\[
a_i^{(f)} = A(2+2\sqrt2)^i + B(2-2\sqrt2)^i - \tfrac{11}{14}.
\]
At this point, we can calculate constants $A, B$ by enforcing the base values $a_0, a_1$:
\[
 A = \frac{7+5\sqrt2}{4},
 \qquad
 B = \frac{7-5\sqrt2}{4}.
\]
Therefore
\begin{equation} \label{eq:legendre-error-bound}
 a_i
 = \frac{7+5\sqrt2}{4}(2+2\sqrt2)^i
 + \frac{7-5\sqrt2}{4}(2-2\sqrt2)^i
 - \frac{11}{14}.
\end{equation}

Since $|2-2\sqrt2|<1$, we can estimate, for a large number of iterations:
\[
 a_i \sim \frac{7+5\sqrt2}{4}(2+2\sqrt2)^i.
\]

In the following table, we calculate the error bound for the first 10 iterations with
the recurrence formula in Equation \ref{eq:legendre-recurrence}. We can see that
the ratios $b_i/b_{i-1}$ converge rapidly to $2+2\sqrt2\approx 4.8284271247\dots$,
the dominant root of Equation \ref{eq:legendre-error-bound}.

\begin{center}
\begin{tabular}{p{.1\textwidth}p{.20\textwidth}p{.20\textwidth}p{.1\textwidth}p{.15\textwidth}}
\hline
$i$ & $b_i$ (exact) & $b_i$ (decimal) & $\lceil\log_2(b_i)\rceil$ & $b_i/b_{i-1}$ \\
\hline
0  & $\frac{1}{2}$         & $0.5$          & -- & --                 \\ [.6em]
1  & $\frac{19}{2}$        & $9.5$          & 4  & $19.0$             \\ [.6em]
2  & $\frac{91}{2}$        & $45.5$         & 6  & $4.789473684\dots$ \\ [.6em]
3  & $\frac{451}{2}$       & $225.5$        & 8  & $4.956043956\dots$ \\ [.6em]
4  & $\frac{2179}{2}$      & $1089.5$       & 11 & $4.831485588\dots$ \\ [.6em]
5  & $\frac{10531}{2}$     & $5265.5$       & 13 & $4.832950895\dots$ \\ [.6em]
6  & $\frac{50851}{2}$     & $25425.5$      & 15 & $4.828696230\dots$ \\ [.6em]
7  & $\frac{245539}{2}$    & $122769.5$     & 17 & $4.828597274\dots$ \\ [.6em]
8  & $\frac{1185571}{2}$   & $592785.5$     & 20 & $4.828442732\dots$ \\ [.6em]
9  & $\frac{5724451}{2}$   & $2862225.5$    & 22 & $4.828433725\dots$ \\ [.6em]
10 & $\frac{27640099}{2}$  & $13820049.5$   & 24 & $4.828427914\dots$ \\ [.6em]
\hline
\end{tabular}
\end{center}
...
Physicist's Hermite polynomials $H_n(x)$, with $n \geq 0$, are widely used in
physics to describe the energy eigenstates (wavefunctions) and energy levels
of the quantum harmonic oscillator (QHO). They are a sequence of orthogonal
polynomials defined in the domain $(-\infty,\infty)$. Unless otherwise stated,
$H_n(x)$ refers to the physicist's convention; this differs from the
probabilist's Hermite polynomials by a simple rescaling of the argument
and normalization.
These polynomials are computed by MPFR through the following recursion:
...
recurrence is not evaluated. These cases are:
\begin{itemize}
\item[(a)] The base cases $H_0$ and $H_1$.
\item[(b)] If $n \ge 1$ is odd, then $H_n(0) = 1$.
\item[(c)] If $n \ge 1$ is even, then $H_n(0) = H_{2k}(0) = (-1)^k~\dfrac{(2k)!}{k!}$.
\end{itemize}
The fraction in (c) is computed with the Log-Gamma method to mitigate the
overflow risk for large $n$. Unfortunately, also Algorithm
\ref{algo:hermite-zero-even-special} is subject to overflow when $n$ is
huge, but as of now, MPFR restricts it to $8192$, i.e., $2^{13}$.
Let's start from the implementation of the case $H_{2k}(0)$ and its
error analysis.
\vspace{.3cm}
...
\begin{algorithm}[H]
  ...
  \BlankLine
  \tcc{The higher degree allowed is $2^{13}$}
  \lIf{$~~(IS\_NAN (x)~||~IS\_INF (x)~||~n>8192)~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
  \lIf{$~~(n=0)~~$}{
  ...
  \If{$~~(x=0)~~$}{
    ...
    $\quad$\lIf{$~~((n\&1) = 0)~~$}{
      \Return zero\_x\_even\_degree (res, n, rnd\_mode)\;
    }
    $\quad$res $\gets$ 0\;
    $\quad$\Return 0\;
  }
  ...
  \lIf{$~~(n=1)~~$}{
    \Return mpfr\_mul\_ui(res, x, 2, rnd\_mode)\;
  }
  \BlankLine
\end{algorithm}
\vspace{.3cm}

\paragraph{\textbf{Implementation of $\mathbf{H_{2k}(0)}$ and error analysis.}}

In Algorithm \ref{algo:hermite-zero-even-special}, you can find the pseudo-code
implementation of the special case $H_{2k}(0)$ with final rounding.
\vspace{.3cm}

\begin{algorithm}[H]
  \caption{\texttt{mpfr\_hermite} $H_{2k}(0)$}
  \label{algo:hermite-zero-even-special}
  \KwData{mpfr\_ptr res, unsigned n, mpfr\_rnd\_t rnd\_mode}
  \KwResult{ternary value}
  \BlankLine
  $\text{first} \gets \N(n+1)$\;
  $\text{second} \gets \N(\frac{n}{2}+1)$\;
  $\text{gamma1} \gets \N(\log(\Gamma(\text{first})))$\;
  $\text{gamma2} \gets \N(\log(\Gamma(\text{second})))$\;
  $\text{sub} \gets \N(\text{gamma1} - \text{gamma2})$\;
  $\text{e} \gets \N(\exp(\text{sub}))$\;
  \tcc{Here we omit both the cancellation and the overflow check}
  \lIf{$~~((n>>1)\&1)~~$}{
    MPFR\_SET\_NEG (res)\;
  }
  \BlankLine
...
Here's the error analysis of the special case $H_{2k}(0)$.
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$\text{gamma1} \gets \N(\log(\Gamma(\text{first})))$
& $k_1 \le \tfrac12$. \\[4pt]
...
& $k_2 \le \tfrac12$. \\[4pt]
$\text{sub} \gets \N(\text{gamma1} - \text{gamma2})$
& $k_3 \le c_3 + k_1 + k_2;~c_3=\frac12$. Thus:
$k_3 \le \frac12 + \frac12 + \frac12 = \frac32$.
A runtime heuristic it's applied to handle eventual cancellation. \\[4pt]
$\text{e} \gets \N(\exp(\text{sub}))$
& $k_4 \le e^{k_3 2^{e_{s}} - p}~2^{e_{s}+1}~k_3$. \\[4pt]
\end{tabular}
\end{center}

\paragraph{\textbf{Recursion implementation and error analysis.}}

In Algorithm \ref{algo:hermite-rec}, you can find the pseudo-code
implementation of the recursion used to compute the (physicist's)
Hermite polynomial with final rounding.
\vspace{.3cm}
...
\begin{algorithm}[H]
  ...
  \KwResult{ternary value}
  \BlankLine
  \tcc{All the variables are initialized to the working precision (realprec)}
  $p_2~\gets{}~1$;
  $~~p_1~\gets~\N(2x)$;
  $~~i~\gets~1$\;
  \tcc{Loop from 1 to $n-1$, where $n$ is the degree of the polynomial}
  \While{$i<n$}{
    $\mathrm{first\_term}~\gets~\N{}(2\cdot{}x)$\;
    $\mathrm{second\_term}~\gets~\N{}(p_2\cdot{}(2 * i))$\;
    $\mathrm{first\_term}~\gets~\N{}(\mathrm{first\_term}\cdot{}p_1)$\;
    $p_i~\gets~\N{}(\mathrm{first\_term} - \mathrm{second\_term})$\;
    \tcc{Here we omit the cancellation check}
    \BlankLine
    $p_2~\gets~p_1$\;
    $p_1~\gets~p_i$\;
    $i++$\;
...
Now, we have to bound the rounding error for the $n$-th iteration of the loop.
This analysis is quite similar to the one performed for Legendre polynomials,
so we won't repeat all the steps here. We just report the main differences.
The only significative difference is the last division, that is missing here.
So we get $k_{p_1^1} \le \frac92$ instead of $k_{p_1^1} \le \frac{19}{2}$.
Here's the error analysis of the first iteration of the loop:
\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$p_2^1 \gets p_1^0$
& $k_{p_2^1} \le k_{p_1^0};\quad k_{p_2^1} \le \frac12$ \\[4pt]
$p_1^1 \gets p_i$
& $k_{p_1^1} \le k_{p_i};\quad k_{p_1^1} \le \frac92$ \\[4pt]
\end{tabular}
\end{center}

\noindent From this, we can now derive the recurrence:
\[
  a_{i+1} \;\le\; 2a_i + 2b_i + \tfrac52, \qquad
  b_{i+1} = a_i.
\]
As we can see, it only differs from the Legendre case in the coefficients.
Substituting $b_i$ yields the second-order recurrence of the form
...
\begin{aligned}
  &a_0 \le \frac12 \\
  &a_1 \le \frac92 \\
  &a_{i+1} \le 2a_i + 2a_{i-1} + \frac52
\end{aligned}
\right.
\end{equation}
As in the previous analysis, to analyze the error growth, we start considering
the homogeneous relation $a_{i+1} \le 2a_i + 2a_{i-1}$, and we look for
solutions of the form $a_i = r^i$.
So, substituting into the recurrence gives $r^{i+1} = 2r^i + 2r^{i-1}$, and
dividing by $r^{i-1}$ (with $r \neq 0$) yields the characteristic equation
$r^2 - 2r - 2 = 0$, and its roots are $r_{1,2} = 1 \pm \sqrt{3}$. The general
solution of the homogeneous recurrence is therefore
\[
  b_i \le A(1+\sqrt3)^i + B(1-\sqrt3)^i,
\]
Substituting the bounds for $b_0$ and $b_1$ gives
\[
  A \le \frac{4 + \sqrt3}{\sqrt3},
  \qquad
  B \le \frac{\sqrt3 - 4}{\sqrt3}.
\]

In order to get the full solution, we first remove the constant term by seeking
a constant particular solution $a_i = C$. Substituting into the recurrence
yields $C = 2C + 2C + \tfrac52 = 4C + \tfrac52$, which implies $C = -\frac56$.
Therefore,
\begin{equation} \label{eq:hermite-error-bound}
  a_i
  \le \frac{4+\sqrt3}{\sqrt3}(1+\sqrt3)^i
     + \frac{\sqrt3-4}{\sqrt3}(1-\sqrt3)^i
     - \tfrac56.
\end{equation}
Since $|1-\sqrt3|<1$, the second exponential term decays exponentially.
Thus, for large $i$,
\[
  a_i \sim \frac{4+\sqrt3}{\sqrt3}(1+\sqrt3)^i,
\]
...
\hline
0  & $\frac{1}{2}$      & $0.5$      & -- & --                 \\ [.6em]
1  & $\frac{9}{2}$      & $4.5$      & 3  & $9.0$              \\ [.6em]
2  & $\frac{25}{2}$     & $12.5$     & 4  & $2.777777777\dots$ \\ [.6em]
3  & $\frac{73}{2}$     & $36.5$     & 6  & $2.92$             \\ [.6em]
4  & $\frac{201}{2}$    & $100.5$    & 7  & $2.753424657\dots$ \\ [.6em]
5  & $\frac{553}{2}$    & $276.5$    & 9  & $2.751243781\dots$ \\ [.6em]
6  & $\frac{1513}{2}$   & $756.5$    & 10 & $2.735985533\dots$ \\ [.6em]
7  & $\frac{4137}{2}$   & $2068.5$   & 12 & $2.734302709\dots$ \\ [.6em]
8  & $\frac{11305}{2}$  & $5652.5$   & 13 & $2.732656514\dots$ \\ [.6em]
9  & $\frac{30889}{2}$  & $15444.5$  & 14 & $2.732330827\dots$ \\ [.6em]
10 & $\frac{84393}{2}$  & $42196.5$	 & 16 & $2.732137654\dots$ \\ [.6em]
\hline
\end{tabular}
\end{center}
