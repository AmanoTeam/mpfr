...
with final rounding. Now, we have to bound the rounding error for each
iteration.

\begin{center}
\begin{tabular}{p{.37\textwidth}p{.63\textwidth}}
$p_1 \gets \N(x)$ & $k_{p_1} \le \tfrac12$.\\[4pt]
$p_2 \gets 1$ &
$k_{p_2}=0$ at initialization. For subsequent iterations we assume
$k_{p_2} \le \tfrac12$. \\[4pt]
...
$k_3 \le c_3 + (1+c_{p_1}^+)k_{\text{first}} + (1+c_{\text{first}}^+)k_{p_1}$,
with $c_3=\tfrac12$ and $c_{p_1}^+=c_{\text{first}}^+=1$.  
Thus $k_3 \le \tfrac12 + 2(k_{p_1}+k_2) = \tfrac12 + 2\Bigl(\tfrac12+\tfrac12\Bigr) = \tfrac52$. \\[4pt]
$\text{second\_term} \gets \N((i-1)p_2)$ &
Integer multiplication followed by rounding:
$k_4 \le c_4 + (1+c_{p_2}^+)k_{p_2}$,
with $c_4=\tfrac12$, $c_{p_2}^+=1$, $k_{p_2}\le \tfrac12$. Thence:
$k_4 \le \tfrac12 + 2k_{p_2} \le \tfrac32.$ \\[4pt]
$p_n \gets \N(\text{first\_term} - \text{second\_term})$ &
Using the generic subtraction rule (same sign operands):
$k_{p_n} \le c_{p_n} + k_3 + k_4, c_{p_n}=\tfrac12$. Thus:
$k_{p_n} \le \tfrac12 + \tfrac52 + \tfrac32 = \tfrac92$.
\textcolor{red}{If cancellation occurs, the effective bound is increased by
a factor $2^d$, with $d=e_{\text{first}}-e_{p_n}$}. \\[4pt]
$p_n \gets \N(p_n/i)$ &
Exact integer division. Using generic division:
$k_{p_n}' \le c_{p_n'} + 2k_{p_n}, c_{p_n'}=\tfrac12$. Thus,
$k_{p_n}' \le \tfrac12 + 2\cdot \tfrac92 = \tfrac{19}{2}$.
\\[4pt]
\end{tabular}
\end{center}
