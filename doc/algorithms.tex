...
\end{eqnarray*}
Since $e^\delta-1$ and $\delta e^\delta$ are of same sign,
and both larger than $|\delta|/2$ in absolute value for $|\delta| \le 0.5$,
we get when $W \ge 0$ (i.e., $x \ge 0$) that
$|\epsilon| \ge x |\delta|/2 (1 + 1/W)$,
thus
\[ |\delta| \le \frac{|\epsilon|}{2x(1+1/W)} \le \frac{|\epsilon|}{2x(1+1/(w+0.5))}, \]
since we have $w = W+\delta$ with $|\delta| \le 0.5$.
This gives us a bound for $|\delta|$ in terms of the residual $\epsilon$, $x$,
and $w$. If this bound is larger than $0.5$, we cannot compute anything.
If it is smaller than $0.5$, it gives a bound on $w-W$.
[FIXME: is that reasoning correct?]
