...
orthogonal polynomials defined in the domain $(-\infty,\infty)$. Unless
otherwise stated, $P_n(x)$ refers to the physicist's convention; this differs
from the probabilist's Hermite polynomials by a simple rescaling of the
argument and normalization. These polynomials are computed by MPFR through
...
\begin{equation} \label{eq:hermite-recursion}
  ...
  \end{cases}
\end{equation}
The polynomial $P_n(x)$ is even for $n$ even, i.e.,
$P_n(-x) = P_n(x)$, and odd for $n$ odd, i.e., $P_n(-x) = -P_n(x)$.
