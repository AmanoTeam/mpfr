...
Legendre polynomials are calculated using a generating and recursive function known as
Bonnet's recursion. Here's the recursion step
\begin{equation} \label{eq:bonnet-recursion}
n P_n(x) = (2n - 1)\, x \, P_{n-1}(x) - (n - 1)\, P_{n-2}(x)
\end{equation}
with two base cases: $P_0(x) = 1$ and $P_1(x) = x$. Some values of x and n are treated
as special cases. These cases are:
\begin{itemize}
\item[(a)] The base cases $P_0$ and $P_1$.
\item[(b)] $P_n(1) = 1$.
\item[(c)] $P_n(-1) = (-1)^n$.
\item[(d)] If $n$ is an odd number greater than 1, then $P_n(0) = 0$.
\end{itemize}

\noindent Equations in (b)--(d) are simple consequences of well known properties of
Legendre polynomials. In particular, (d) is a straightforward consequence of Equation
\ref{eq:legendre-parity-eq}, that establish the parity rule of Legendre polynomials.
\begin{equation} \label{eq:legendre-parity-eq}
  P_n(-x) = (-1)^nP_n(x)
\end{equation}
Equations (b) and (c) can be proven by induction from Bonnet recursion formula
(Equation \ref{eq:bonnet-recursion}).
...
\begin{algorithm}[H]
  ...
  assert(x not NULL)\;
  \BlankLine
  \tcc{First we have to check the parameters.}
  \lIf{$~~(n<0)~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
...
\noindent In Algorithm \ref{algo:legendre-bonnet} below, we can find the pseudo--code
implementation of the Bonnet's recursion with final rounding.
\vspace{.5cm}

\begin{algorithm}[H]
  \caption{\texttt{mpfr\_legendre} Bonnet's recursion}
  \label{algo:legendre-bonnet}
  mpfr\_init2 (first\_term, realprec)\;
  mpfr\_init2 (second\_term, realprec)\;
  mpfr\_init2 ($p_2$, realprec)\;
  mpfr\_init2 ($p_1$, realprec)\;
  mpfr\_init2 ($p_n$, realprec)\;
  \BlankLine
  ...
  $i~\gets~2$\;
  \While{$i\leq{}n$}{
    $\mathrm{first\_term}~\gets~\nabla{}(2\cdot{}i\cdot{}x-1)$\;
    $\mathrm{second\_term}~\gets~\nabla{}(p_2\cdot{}i-1)$\;
    $p_n~\gets~\nabla{}(\mathrm{first\_term}\cdot{}p_1 - \mathrm{second\_term})$\;
    $p_n~\gets~\nabla{}(p_n/i)$\;
    \BlankLine
    $p_2~\gets~p_1$\;
    $p_1~\gets~\nabla{}(p_n)$\;
  ...
  $\mathrm{round}(\circ(p_n), \mathrm{prec}(\mathrm{res}))$\;
  $\mathrm{res}~\gets~\circ{}(p_n)$\;
  \BlankLine
\end{algorithm}
