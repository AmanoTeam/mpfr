...
\label{subsec:legendre-polynomials}
...
\label{subsec:legendre-small-inputs}
...
\label{par:legendre-error-analysis}
...
\label{subsec:hermite-polynomials}

Hermite polynomials $P_n(x)$, with $n \geq 0$, are a sequence of orthogonal
polynomials defined in the domain $(-\infty,\infty)$. These have a special
interest in various branches of physics, e.g. in defining the eigenstates
of the quantum harmonic oscillator.

In this document, $P_n(x)$ refers to the physicist's convention -- the
ones implemented by \texttt{mpfr\_hermite}; this differs from the
probabilist's convention, which is often denoted by $H_n(x)$ and is
related to $P_n(x)$ by a scaling factor.

MPFR does not use the Rodrigues formula to compute Hermite polynomials
explicitly, but instead relies on a three-term recursion, which is more
efficient and numerically stable for this purpose.
\begin{equation} \label{eq:hermite-recursion}
  \begin{cases}
      P_0(x) = 1,  \\
      P_1(x) = 2x, \\
      P_{n+1}(x) = 2xP_{n}(x) - 2nP_{n-1}(x).
  \end{cases}
\end{equation}
The polynomial $P_n(x)$ is even for $n$ even, and odd for $n$ odd,
i.e., $P_n(-x) = (-1)^n~P_n(x)$.

Some values of $x$ and $n$ are treated as special cases in which the
recurrence is not evaluated. These two cases are, where (b) follows
from the fact that $P_n$ is odd for $n$ odd:
\begin{itemize}
\item[(a)] The base cases $P_0$ and $P_1$.
\item[(b)] If $n$ is odd, then $P_n(0) = 0$.
\end{itemize}

\paragraph{\textbf{Recursion implementation and error analysis.}}

Let's start from Algorithm \ref{algo:hermite-special} in which special cases
are implemented.
\vspace{.3cm}

\begin{algorithm}[H]
  \caption{\texttt{mpfr\_hermite} special cases}
  \label{algo:hermite-special}
  \KwData{mpfr\_ptr res, unsigned n, mpfr\_srcptr x, mpfr\_rnd\_t rnd\_mode}
  \KwResult{ternary value}
  \BlankLine
  \lIf{$~~(IS\_NAN (x)~||~IS\_INF (x))~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
  \lIf{$~~(n=0)~~$}{
  ...
  \lIf{$~~(\text{MPFR\_IS\_SINGULAR} (x))~~$}{
    res $\gets$ NaN;
    \Return 0\;
  }
  \lIf{$~~(n=1)~~$}{
    \Return mpfr\_mul\_2ui(res, x, 1, rnd\_mode)\;
  }
  \BlankLine
\end{algorithm}
\vspace{.3cm}
...
\begin{algorithm}[H]
  \caption{\texttt{mpfr\_hermite} recursion}
  \label{algo:hermite-rec}
  \KwData{mpfr\_ptr res, unsigned n, mpfr\_srcptr x, mpfr\_rnd\_t rnd\_mode}
  \KwResult{ternary value}
  \BlankLine
  \tcc{All the following MPFR numbers are initialized to precision $p$}
  $b~\gets{}~1$;
  $~~a~\gets~\N{}(2x)$;
  $~~i~\gets~1$\;
  \While{$i<n$}{
...
Algorithm \ref{algo:hermite-rec} shows how the recursion for Hermite polynomial
is implemented in MPFR. In the error analysis below we will name $a_i$ and
$b_i$ the values of the variables $a$ and $b$ at the start of loop $i$, where
$a_i$ approximates $P_i(x)$ and $b_i$ approximates $P_{i-1}(x)$, and $f_i$,
$g_i$, $h_i$, $q_i$, $b_{i+1}$, $a_{i+1}$ the values of the variables $f$, $g$, $h$,
$q$, $b$, $a$ computed during loop $i$. We are interested in the final value
$a_n$, which approximates $P_n(x)$.

For the error analysis, we are going to use the same naming convention that
we used for Legendre polynomials in Section
\ref{subsec:legendre-polynomials}, so we will associate to each variable,
say $x_i$ at loop $i$, an integer $e_{x,i}$ such that the total rounding
error on $x_i$ with respect to the exact value $X_i$ it approximates is
bounded as follows:
\[ |x_i - X_i| \le 2^{e_{x,i}}. \]
This error bound $e_{x,i}$ will be computed at runtime within the code
(due to the possible cancellation in $q = \N{}(h - g)$, it is not possible
to compute an error bound \textit{a priori}).

As for Legendre polynomials, we are going to assume all roundings are to
nearest, with working precision p.

\par\noindent
\textbf{Analysis of $a = \N{}(2\cdot{}x)$.} This is error bound of the
first initialization of the value $a$, say $a_1$, which approximates $P_1(x)=2x$.
Here, we use the
\texttt{mpfr\_mul\_2ui} function to multiply the exact input $x$ by 2.
Thus, we just have to consider the error due to the final rounding
$\N{}(2\cdot{}x)$. (If the precision of $a$ is less than that of $x$,
the multiplication $a = \N{}(2\cdot{}x)$ can yield a rounding error.)
This error is bounded by
$\frac{1}{2} \ulp(a_1) \le \frac{1}{2} 2^{\Exp(a_1) - p}$:
\[ e_{a_1} \le \Exp(a_1) - p - 1. \]

\par\noindent
\textbf{Analysis of $f_i = \N{}(2\cdot{}x)$.} As for $a_1$, there is only
one error, due to the final rounding. This error is bounded by
$\frac{1}{2} \ulp(f_i) \le \frac{1}{2} 2^{\Exp(f_i) - p}$:
\[ e_{f,i} \le \Exp(f_i) - p - 1. \]

\par\noindent
\textbf{Analysis of $g_i = \N{}(b_i \cdot{}(2i))$.}
Here we have two sources of error: the rounding error on $g_i$, and
the induced error on $b_i$. The first one is bounded by
$\frac{1}{2} \ulp(g_i)$, the second one by $2^{e_{b,i}} (2i)$.
Since $2^x+2^y \le 2^{\max(x,y)+1}$:
\[ e_{g,i} \le \max(\Exp(g_i)-p, e_{b,i} + \lceil \log_2(2i) \rceil + 1). \]

\par\noindent
\textbf{Analysis of $h_i = \N{}(f_i\cdot{}a_i)$.}
Here we have three sources of error: the rounding error on $h_i$,
the induced error on $f_i$, which approximates $2x$,
and the induced error on $a_i$, which approximates $P_i(x)$:
\begin{eqnarray*}
  |h_i - 2x P_i(x)| & \le &
  |h_i - f_i a_i| + |f_i a_i - 2x a_i| + |2x a_i - 2x P_i(x)| \\
  & \le & 2^{\Exp(h_i)-p-1} + 2^{e_{f,i} + \Exp(a_i)} + 2^{1 + \Exp(x) + e_{a,i}}.
\end{eqnarray*}
Since $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{h,i} \le 2 + \max(\Exp(h_i)-p-1, e_{f,i} + \Exp(a_i),
  1 + \Exp(x) + e_{a,i}). \]

\par\noindent
\textbf{Analysis of $q_i = \N{}(h_i - g_i)$.}
Here again, we have three sources of error: the rounding error on $q_i$,
the induced error on $h_i$, which approximates $H_i := 2x P_i(x)$,
and the induced error on $g_i$, which approximates $G_i := (2i)P_{i-1}(x)$:
\[ |q_i - (H_i - G_i)| \le 2^{\Exp(q_i)-p-1} + 2^{e_{h,i}} + 2^{e_{g,i}}. \]
Using again $2^x + 2^y + 2^z \le 2^{\max(x,y,z)+2}$, it follows:
\[ e_{q,i} \le 2 + \max(\Exp(q_i)-p-1, e_{h,i}, e_{g,i}). \]

At the end of the loop, we will get an approximation $a_n$ of $P_n(x)$ such
that:
\[ |a_n - P_n(x)| \le 2^{e_{a,n}}. \]
This error bound will be used in the rounding test, and if it fails, we will try
again with a larger working precision $p' > p$.

\paragraph{\textbf{Overflow handling.}}

In this paragraph it's shown how overflow is handled in \texttt{mpfr\_hermite}.
Each polynomial $P_n$ has $n$ distinct real roots, each of them lies in the
closed interval $[-\sqrt{2n+1}, \sqrt{2n+1}]$, that is called the
\emph{oscillatory region} of the polynomial $P_n(x)$.

The values of $P_n(x)$ grow very rapidly as $|x|$ increases beyond $\sqrt{2n+1}$,
and thus overflow can occur for large $n$ and sufficiently large $|x|$.
The following monotonic growth theorem shows that, outside the oscillatory region,
if an overflow occurs at some step of the recursion, it will occur at
all subsequent steps, i.e. if $P_n(x)$ overflows, the same happens to every
$P_i(x)$ with $i > n$.

\noindent Consider the recurrence $P_{n+1}(x) = 2x P_n(x) - 2n P_{n-1}(x)$.

\begin{lemma}
For $x > 0$ with $x^2 > 2n$, and for all $1 \le i \le n$:
\[
  P_i(x) / P_{i-1}(x) > x.
\]
\end{lemma}

In other words, at each step of the recurrence the value grows by at
least a factor $x$. In particular $P_i(x) > x^i > 0$, and the sequence
$|P_i(x)|$ is strictly increasing with $i$. This means that, outside
the oscillatory region, i.e. $x^2 > 2i + 1$, if $P_i(x)$ overflows,
the same does $P_{i+1}(x)$.

\begin{proof}{(by induction on $i$)}
For the base case $i = 1$: $P_1(x)/P_0(x) = 2x > x$ since $x > 0$.
For the inductive step, assume $P_i(x)/P_{i-1}(x) > x$ for some
$i \ge 1$ with $x^2 > 2i$.
From the recurrence:
\[
  \frac{P_{i+1}(x)}{P_i(x)} = 2x - 2i \, \frac{P_{i-1}(x)}{P_i(x)}.
\]

By the induction hypothesis, $P_i(x)/P_{i-1}(x) > x > 0$, hence
$0 < P_{i-1}(x)/P_i(x) < 1/x$. Therefore:
\[
  \frac{P_{i+1}(x)}{P_i(x)} > 2x - \frac{2i}{x}.
\]
This is greater than $x$ if and only if $x > 2i/x$, i.e., $x^2 > 2i$,
which holds by hypotesis. This completes the induction.
\end{proof}

\begin{corollary}
If $x > 0$ with $x^2 > 2n$, and if $P_i(x)$ overflows for some
$1 \le i \le n$, then $P_m(x)$ overflows for all $m$ with
$i \le m \le n$.
\end{corollary}

In the implementation, computations are performed in extended exponent
range (after \texttt{MPFR\_SAVE\_EXPO\_MARK}). An overflow in this range
means the value exceeds \texttt{MPFR\_EMAX\_MAX}.

If $|x| \leq \sqrt{2n+1}$ the polynomial changes sign and its values can
decrease in magnitude from one step to the next, so the monotonic growth
argument does not hold within this interval. At the moment, MPFR treats every
intermediate overflow in the same way, without considering whether the
computation is performed in the oscillatory region or not.

\subsubsection{Handling small inputs}

As for Legendre polynomials, when $|x|$ is very small, $P_n(x)$ can be well
approximated by the leading term of its Taylor expansion around $x=0$. Thus,
we are going to use the same methodology used for Legendre polynomials in Section
\ref{subsec:legendre-small-inputs} to bound the ratios of the first terms
($|c_3/c_1|$ for odd polynomials and $|c_2/c_0|$ for the even ones). Starting
from the Hermite differential equation $y'' - 2x y' + 2ny = 0$ we obtain the
following recurrence
\begin{equation} \label{eq:hermite-ode-recurrence}
  (k+2)(k+1)c_{k+2}
  + 2 (n - k) c_k = 0.
\end{equation}

For odd degrees, we set $k = 1$, and we get $(3)(2)c_3 = -2(n-1)c_1$. Hence,
\[
  \left|\frac{c_3}{c_1}\right| = \frac{n-1}{3} \le \frac{n}{3}.
\]

Now, for even degrees we set $k = 0$ and we get $(2)(1)c_2 = -2n\,c_0$. Hence,
\[
  \left|\frac{c_2}{c_0}\right| = n.
\]

For generic terms, Equation (\ref{eq:hermite-ode-recurrence}) gives
\[
  \frac{c_{k+2}}{c_k} = \frac{-2(n-k)}{(k+2)(k+1)},
\]
so for $0 \le k < n$:
\[
  \left|\frac{c_{k+2}}{c_k}\right|
  =
  \frac{2(n-k)}{(k+2)(k+1)}
  \le \frac{2n}{2} = n.
\]
For $k \ge n$, $c_{k+2} = 0$. Therefore, $|c_{k+2}/c_k| \le n$ for all $k \ge 0$.
Let
\[
  t := nx^2,
\]
then the tail is bounded by a geometric series:
\[
  \left|\sum_{j \ge 1} c_{s+2j}x^{s+2j}\right|
  \le |c_s x^s|\,\sum_{j \ge 1} t^j
  = |c_s x^s|\,\frac{t}{1-t}
\]
for $t < 1$.
...
\item If $n=2m$ is even, the leading term is $c_{0,m} = P_n(0)$, computed by
  \[
    c_{0,0}=1, \qquad c_{0,j}=c_{0,j-1}\,\big(-2(2j-1)\big),
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_{0,m} = \prod_{j=1}^{m} \big(-2(2j-1)\big).
  \]

\item If $n=2m+1$ is odd, the leading term is $c_1x$ with
  $c_{1,m}=P_n'(0)$, computed by
  \[
    c_{1,0}=2, \qquad c_{1,j}=c_{1,j-1}\,\big(-2(2j+1)\big),
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_{1,m} = 2\prod_{j=1}^{m} \big(-2(2j+1)\big).
  \]
\end{itemize}
We remark that $c_{1,m} = 2(2m+1)c_{0,m}$.

\paragraph{\textbf{Bounding $t$ in the implementation.}}
In the code, $t = nx^2$ is bounded as follows.
Let $e_x = \texttt{MPFR\_GET\_EXP}(x)$, which satisfies
$2^{e_x-1} \le |x| < 2^{e_x}$, and let
$\ell = \lceil \log_2 n \rceil$, so that $n \le 2^\ell$.
Then
\[
  t = nx^2 < 2^\ell \cdot \left(2^{e_x}\right)^2
          = 2^{\ell + 2e_x} = 2^\rho,
\]
where $\rho = 2e_x + \ell$.
...
\paragraph{\textbf{Error analysis of $c_{0,m}$ and $c_{1,m}$}}
As for the Legendre case (Sections \ref{par:legendre-error-analysis}), we set
a working precision $q$, and each \texttt{mpfr\_mul\_si} introduces a relative
error of at most $u = 2^{-q}$ per operation. We denote by $V_k$ the exact real
value of the $k$-th iterate and by $v_k$ its computed floating-point approximation,
with $V_0 = v_0 = 1$ exact in the even case, and $V_0 = v_0 = 2$ exact in the odd
case. After $k$ such rounding
operations the accumulated relative error satisfies
\[
  \frac{|v_k - V_k|}{|V_k|}
  \le \frac{ku}{1-ku}
  \le 2ku = k \cdot 2^{1-q},
...
\subsection{The Lambert W function}

The Lambert W function $W(z)$ is defined as the inverse of $W(z) = ze^z$, where
$z$ is a complex number, i.e., it satisfies $w e^w = z$ where $w = W(z)$
\cite{DBLP:journals/na/Johansson20}.
Since $W(z)$ is a complex multivalued function, we only
consider its two real branches here: $W_0$ (often called the \textit{principal}
branch), and $W_{-1}$. $W_0$ is defined for $x \ge -1/e$, while $W_{-1}$ is defined
for $-1/e \le x < 0$; see Figure~\ref{fig:lambertw}. The two branches meet at
the point $x = -1/e$, where $W_0(-1/e) = W_{-1}(-1/e) = -1$, and for $x < -1/e$
the function is not defined in the real domain.

\begin{figure}[htp]
\centering
\includegraphics[width=.9\textwidth]{lambert_w_plot.pdf}
\caption{The two real branches of the LambertW function: the principal
  branch $W_0$ (defined for $x \ge -1/e$) and the branch $W_{-1}$
  (defined for $-1/e \le x < 0$). Both branches meet at $(-1/e, -1)$;
  $W_0(-1/e) = W_{-1}(-1/e) = -1$.}
\label{fig:lambertw}
\end{figure}

\subsubsection{The main branch $W_0$}
\label{subsubsec:w0}

\paragraph{\textbf{Initial guess}}
\label{par:w0-initial-guess}

The initial guess is computed by MPFR in different ways, depending on the
magnitude of $x$. When $x$ is the range $[-1/e, 1066510.2]$, the initial guess
is calculated by a polynomial approximation. In particular, this range is
divided in 16 subranges. For each of those, on
Table~\ref{tab:w0-poly-initial-guess} are reported the boundaries $[a,b]$, and
the maximum error $|W_0(x)-P(x)|$, where $P(x)$ is the polynomial interpolating
$W_0$ in the specified range $[a,b]$. All the coefficients are stored in a
precomputed table in the code and evaluated with Horner's method. The hardest
segment to approximate is the closest to the boundary $-1/e$ (segment 0 in
Table~\ref{tab:w0-poly-initial-guess}). For that one we use the same polynomial
as for segment 1, that produces an error of magnitude
$10^{-2}$. Starting from segment 3, the width of each interval $w_k$ is
calculated by the geometric progression $w_k = 0.2 \cdot j^k$,
with $k = {\rm segment} - 3$ and $j \approx 3.5363$,
where the value of $j$ is chosen empirically to guarantee
the error for $|W_0(x) - P_k(x)| < 10^{-5}$ for each $w_k$. For more details
about the sub-ranges boundaries and the maximum error, see segments 3--15 in
Table~\ref{tab:w0-poly-initial-guess}.

\begin{table}[htp]
\begin{tabular}{cccc}
segment & a           & b            & $|W_0(x)-P(x)|$ \\[0.05cm]
\hline
0       & $-1/e$      & -0.36        & ?               \\[0.05cm]
1       & -0.36       & -0.35        & 6.3398e-09      \\[0.05cm]
2       & -0.35       & -0.28321206  & 2.9263e-06      \\[0.05cm]
3       & -0.3        & -0.1         & 1.8725e-06      \\[0.05cm]
4       & -0.1        & 0.60725964   & 2.0653e-06      \\[0.05cm]
5       & 0.60725964  & 3.1083407    & 3.0241e-06      \\[0.05cm]
6       & 3.1083407   & 11.952909    & 4.3717e-06      \\[0.05cm]
7       & 11.952909   & 43.22994     & 5.7751e-06      \\[0.05cm]
8       & 43.22994    & 153.83485    & 6.9773e-06      \\[0.05cm]
9       & 153.83485   & 544.9668     & 7.8947e-06      \\[0.05cm]
10      & 544.9668    & 1928.126     & 8.5598e-06      \\[0.05cm]
11      & 1928.126    & 6819.3895    & 9.0376e-06      \\[0.05cm]
12      & 6819.3895   & 24116.356    & 9.3856e-06      \\[0.05cm]
13      & 24116.356   & 85283.587    & 9.6453e-06      \\[0.05cm]
14      & 85283.587   & 301589.16    & 9.8441e-06      \\[0.05cm]
15      & 301589.16   & 1066510.2    & 9.9999e-06      \\[0.05cm]
\hline                                                 \\
\end{tabular}
\caption{In this table are presented all the ranges $[a,b]$ that we
  used to calculate the initial guess of the Lambert W function
(branch $W_0$).}
\label{tab:w0-poly-initial-guess}
\end{table}

\paragraph{\textbf{Halley's iteration}}
The initial guess is refined with Halley's iteration. An Halley step computes
$f(w) = we^w - x$. Its derivatives are $f'(w) = (w+1)e^w$ and $f''(w) =
(w+2)e^w$. Starting from the initial guess $w_0$ (see Paragraph ``Initial
guess'' in Section \ref{par:w0-initial-guess}), we compute $w_{n+1}$ with the
iteration in Equation~(\ref{eq:lambertw-halley}).

\begin{equation} \label{eq:lambertw-halley}
  w_{n+1} = w_n - \dfrac{f(w_n)}{f'(w_n) - \frac{f(w_n) f''(w_n)}{2 f'(w_n)}}
         = \dfrac{f(w_n)}{(w_n+1)e^{w_n} - \frac{(w_n+2) f(w_n)}{2(w_n+1)}}.
\end{equation}

Assume we have computed an approximation $w$ of $W(x)$ with residual
$w e^w - x = \epsilon$, and we want to deduce a bound on the difference
$w - W$, where $W = W(x)$. Write $w = W + \delta$. Then:
\begin{eqnarray*}
\epsilon &=& w e^w - W e^W = (W+\delta) e^{W+\delta} - W e^W \\
         &=& W e^W (e^\delta-1) + \delta e^\delta e^W
             = x (e^\delta - 1 + \frac{\delta e^\delta}{W}).
\end{eqnarray*}
We assume $W \ge 0$ here, thus $x \ge 0$.
First consider $\epsilon \ge 0$, which implies $\delta \ge 0$.
We then have $\epsilon/x = e^\delta-1 + \delta e^\delta/W
\ge \delta + \delta/W$, thus $\delta \le \epsilon/x/(1+1/W)$.
Thus if $|\epsilon/x| \le 1/2$, then $\delta \le 1/2$.

Now consider $\epsilon \le 0$, which implies $\delta \le 0$.
Define $\alpha$ such that
$e^\delta-1+\delta e^\delta/W = \alpha (1+1/W)$,
where $\alpha$ is a function of $\delta$ and $W$.
It is easy to see that for $W$ fixed,
$\alpha$ is non-negative and decreasing with $\delta$.
For $\delta=-1/2$, we have $\alpha = g(W)$, where:
\[ g = \frac{e^{-1/2}-1 - \frac{1}{2} e^{-1/2}/W}{1+1/W}, \]
The function $g(W)$ is decreasing on $[0, \infty)$, and $g(0) \approx -0.3032$.
Assume $\epsilon/x > -0.303$, then $\epsilon/x/(1+1/W) = \alpha > -0.303$.
Since $\alpha$ is decreasing with $\delta$, and is smaller than
$-0.303$ for $\delta=-1/2$, this implies $\delta \ge -1/2$.

Whatever the sign of $\epsilon$, 
we conclude that if $|\epsilon/x| < 0.303$, then $|\delta| \le 1/2$.
If $-1/2 \le \delta \le 0$, we have
$|e^\delta-1| \ge 0.786 |\delta|$ and $|\delta e^\delta| \ge 0.606 |\delta|$,
thus $|\epsilon/x| \ge 0.786 |\delta| + 0.606 |\delta|/W$,
and $|\delta| \le |\epsilon|/x/(0.786 + 0.606/W)$.
In both cases, as soon as $|\epsilon/x| < 0.303$, we conclude
in the case $x \ge 0$, using the fact that $w = W + \delta$,
\[ |\delta| \le \frac{|\epsilon|}{x (0.786 + 0.606/(w+1/2))}. \]
This can be used as stopping criteria for Halley's iteration: as soon as
the bound for $|\delta|$ is smaller than $\ulp(w)$ (assuming $w$ has some
guard bits), we can stop the iteration and perform the rounding test.

\subsubsection{Handling small inputs}

When $|x|$ is very small, $W_0(x)$ can be well approximated by the leading
term of its Taylor expansion around $x=0$, avoiding the Halley iteration.
The expansion is
\begin{equation} \label{eq:w0-taylor}
  W_0(x) = \sum_{k=1}^{\infty} \dfrac{(-k)^{k-1}}{k!}\;x^k,
\end{equation}
i.e., $W_0(x) = x - x^2 + \frac{3}{2}x^3 - \frac{8}{3}x^4 + \cdots$, which
converges for $|x| < 1/e$. Starting from the Taylor expansion in Equation
\ref{eq:w0-taylor}, we bound the ratio of two consecutive coefficients of
$|c_k/c_{k+1}|$ as
\[
  \left|\frac{c_{k+1}}{c_k}\right|
  = \frac{(k+1)^k}{(k+1)!} \cdot \frac{k!}{k^{k-1}}
  = \frac{(k+1)^{k-1}}{k^{k-1}}
...
Since $|c_{k+1}/c_k| < e$, each term $c_{k+1}$ is at most a factor $e|x|$ of the
previous $c_k$. As usual, let $t := e|x|$, then the tail is bounded by a
geometric series:
\[
  \left|\sum_{k \ge 2} c_k x^k\right| \le |x|\,\sum_{j \ge 1} t^j
  = |x|\,\frac{t}{1-t}
\]
for $t < 1$.

\paragraph{\textbf{Bounding $t$ in the implementation.}}
Let $\Exp(x) = \texttt{MPFR\_GET\_EXP}(x)$, which satisfies $2^{\Exp(x)-1} \le
|x| < 2^{\Exp(x)}$. Since $e < 2^2$, $t = e|x| < 2^{\Exp(x) + 2} = 2^\rho$,
where $\rho = \Exp(x) + 2$.

The condition $\rho \le -2$ guarantees
$t < 2^\rho \le 2^{-2} = \tfrac{1}{4}$ (in particular $|x| < 1/e$, so the
series converges).
The relative error from keeping only the leading term is bounded by
$t/(1-t)$. Since $t < \tfrac{1}{4} < \tfrac{1}{2}$, we have
$1 - t > \tfrac{1}{2}$, and therefore
\[
  \frac{t}{1-t} < 2t < 2^{\rho+1}.
\]
Hence $|W_0(x) - x| < |x| \cdot 2^{\rho+1} < 2^{\Exp(x) - \mathtt{err}}$ with
$\mathtt{err} = -\rho - 1 = -\Exp(x) - 3$, which is the form of error bound
expected by \texttt{mpfr\_round\_near\_x}.

If $p = \texttt{MPFR\_PREC}(\mathrm{res})$, we try this shortcut only when
$-\rho - 1 \ge p + 64$, i.e., $\Exp(x) \le -p - 67$.

This implementation of the error bounds for small inputs is similar to the one
presented for orthogonal polynomials (see Sections
\ref{subsec:legendre-polynomials} and \ref{subsec:hermite-polynomials}
above). As in these cases, the extra $64$ guard bits are used as an heuristic.
