...
\begin{algorithm}[H]
  ...
  $p_2~\gets{}~1$; \Comment{$p_2$ stores $p_{i-2}$ and $p_1$ stores $p_{i-1}$}
  $p_1~\gets~\N(x)$\;
  $i~\gets~2$\;
...
$n$-th iteration of the loop $\epsilon_n$. At each stage, we assume the
total error for a given approximation $x$ is bounded by $k_x$ ulps.
