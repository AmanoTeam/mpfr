...
\begin{algorithm}[H]
  ...
  \KwData{mpfr\_ptr res, int n, mpfr\_srcptr x, mpfr\_rnd\_t rnd\_mode}
...
\begin{algorithm}[H]
  ...
  \KwResult{ternary value}
  \BlankLine
  realprec $\gets$ mpfr\_get\_prec(res) + 8\;
  \tcc{Initializing all the variables to the working precision}
...
A runtime heuristic it's applyed to handle eventual cancellation. \\[4pt]
$p_i \gets \N(p_i/i)$ &
...
\begin{equation} \label{eq:hermite-recursion}
  \begin{cases}
      H_0(x) = 1\\
      H_1(x) = 2x\\
      H_n(x) = H_{n+1}(x) = 2xH_n(x) - 2nH_{n-1}(x)
  \end{cases}
\end{equation}
