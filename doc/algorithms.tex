...
\subsection{The LambertW function}

The LambertW function $W(z)$ is defined as the inverse of $W(z) = ze^z$, where
$z$ is a complex number. Since $W(z)$ is a complex multivalued function, we only
consider its two real branches here: $W_0$ (often called the \textit{principal}
branch), and $w_{-1}$. $W_0$ is defined for $x \ge -1/e$, while $W_{-1}$ defined
...
that we used for segment 1, that produces an error of magnitude
$10^{-2}$. Starting from $x = -0.3$, the width of each interval $w_k$ is
calculated by the geometric progression $w_k = 0.2 \cdot j^k$, with $j =
3.536298222377439$, where the value of $j$ is chosen empirically to guarantee
...
\caption{In this table are presented all the ranges $[a,b]$ that we
  used to calculate the initial guess.}
\label{tab:w0-poly-initial-guess}
\end{table}

\paragraph{\textbf{Halley iteration}}
